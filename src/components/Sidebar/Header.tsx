...
import { Box, Button, IconButton, InputBase, Paper, CircularProgress } from "@mui/material";
...
export default function Header() {
  ...
  const [searchQuery, setSearchQuery] = useState(""); // Triggers API call on Enter
  ...
  const addNoteMutation = useMutation({
    ...
    onSuccess: (newNote) => {
      ...
      router.push(`/folder/${pathSegments[2]}/${folderId}/notes/${newNote.id}`);
    },
  });
    ...
    <Box p={1}>
      <Box display="flex" justifyContent="space-between" alignItems="center">
        ...
        <IconButton onClick={() => setIsSearching(!isSearching)}>
          {isSearching ? <CloseIcon sx={{ color: "white" }} /> : <SearchIcon sx={{ color: "white" }} />}
        </IconButton>
      </Box>
          ...
          {isLoading ? (
            ...
            notes.map((note: NoteType) => (
              <Box
                ...
                onClick={() => {
                  router.push(`/folder/${note.folder.name}/${note.folderId}/notes/${note.id}`);
